      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-10">
          <div>
            ...
            <h1 className="text-5xl font-bold mb-2">Developer Dashboard</h1>
            <p className="text-zinc-400">Create and manage your properties</p>
          </div>
          <Link href="/developer/create-property" className="bg-[#C8A76A] text-black px-6 py-3 rounded-xl font-semibold">
            + Create Property
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {properties.length === 0 ? (
            ...
            properties.map((p) => (
              <a href={`/developer/property/${p.id}`} key={p.id} className="bg-[#151515] border border-zinc-800 rounded-3xl overflow-hidden block hover:border-[#C8A76A] transition-colors">
                {p.thumbnail_url && (
